...
public class Student {
    ...
    private static Course getRandomCourse(Course... courses){
        return courses[random.nextInt(0,courses.length)];
    }
    public static Student getRandomStudent() {
        Course pymc= new Course("PYMC", "Python Masterclass", 50);
        Course jmc= new Course("JMC", "Java Masterclass", 100);
        Course gamesJmc = new Course("GJMC", "Creating Games In Java",90);

        Course[] courses = new Course[random.nextInt(1,4)];
        int index = 0;
        while(index < courses.length){
            Course randomCourse = getRandomCourse(pymc,jmc,gamesJmc);
            if(Arrays.stream(courses).noneMatch(c -> c == randomCourse)){
                courses[index] = randomCourse;
                index++;
            }
        }
